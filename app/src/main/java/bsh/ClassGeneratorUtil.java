 ...
 *
 * <p>TODO: We have hooks for generating static initializer code, now used to save persistent class
 * stubs. This must be extended to accommodate general static initializer blocks.
 *
 * @author Pat Niemeyer
 */
/*
...
public class ClassGeneratorUtil implements Constants {
    /**
     * The name of the static field holding the reference to the bsh static This (the callback
     * namespace for static methods)
     */
    static final String BSHSTATIC = "_bshStatic";

    /**
     * The name of the instance field holding the reference to the bsh instance This (the callback
     * namespace for instance methods)
     */
    static final String BSHTHIS = "_bshThis";

    /**
     * The prefix for the name of the super delegate methods. e.g. _bshSuperfoo() is equivalent to
     * super.foo()
     */
    static final String BSHSUPER = "_bshSuper";

    /**
     * The name of the instance initializer in the bsh static class namespace. The instance
     * initializer contains instance blocks and loose code, etc.
     */
     ...
     *
     * <p>This method must be called to initialize the static space even if we are using a
    ...
    private void splitMethodsAndConstructors(String className, DelayedEvalBshMethod[] bshmethods) {
        ...
        for (int i = 0; i < bshmethods.length; i++) {
            ...
            if (bm.getName().equals(classBaseName) && bm.getReturnType() == null) consl.add(bm);
            else methodsl.add(bm);
        }
    ...
    public byte[] generateClass(boolean generateInitCode) {
        ...
        for (int i = 0; i < vars.length; i++) {
            ...
            int modifiers;
            if (isInterface) modifiers = ACC_PUBLIC | ACC_STATIC | ACC_FINAL;
            else modifiers = getASMModifiers(vars[i].getModifiers());
        ...
        for (int i = 0; i < methods.length; i++) {
            ...
            boolean overridden =
                    classContainsMethod(
                            superClass, methods[i].getName(), methods[i].getParamTypeDescriptors());
    ...
    /**
     * Generate a delegate method - static or instance. The generated code packs the method
     * arguments into an object array (wrapping primitive types in bsh.Primitive), invokes the
     * static or instance namespace invokeMethod() method, and then unwraps / returns the result.
     */
    static void generateMethod(
        ...
        if (isStatic) pushBshStatic(fqClassName, className, cv);
        else {
            // Push 'this'
        ...
        cv.visitMethodInsn(
                INVOKESTATIC, "bsh/Primitive", "unwrap", "(Ljava/lang/Object;)Ljava/lang/Object;");
    ...
    void generateStaticInitializer(ClassWriter cw) {
        CodeVisitor cv = cw.visitMethod(ACC_STATIC, "<clinit>", "()V", null /*exceptions*/);
        ...
        cv.visitMethodInsn(
                INVOKESTATIC, "bsh/ClassGeneratorUtil", "initStatic", "(Ljava/lang/Class;)V");
     ...
     * constructor signature (if any) and evalute the arguments at runtime. The getConstructorArgs()
    ...
    */
    ...
    static void generateSuperDelegateMethod(
        ...
        // Add method body
        CodeVisitor cv =
                cw.visitMethod(modifiers, "_bshSuper" + methodName, methodDescriptor, exceptions);

        cv.visitVarInsn(ALOAD, 0);
        // Push vars
        int localVarIndex = 1;
        for (int i = 0; i < paramTypes.length; ++i) {
            if (isPrimitive(paramTypes[i])) cv.visitVarInsn(ILOAD, localVarIndex);
     ...
     *
     * <p>Evaluate the arguments (if any) for the constructor specified by the constructor index.
     * Return the ConstructorArgs object which contains the actual arguments to the alternate
     * constructor and also the index of that constructor for the constructor switch.
     *
     * @param consArgs the arguments to the constructor. These are necessary in the evaluation of
     *     the alt constructor args. e.g. Foo(a) { super(a); }
     * @return the ConstructorArgs object containing a constructor selector and evaluated arguments
     *     for the alternate constructor
     */
        ...
        for (int i = 0; i < consArgs.length; i++) {
            try {
                consArgsNameSpace.setTypedVariable(
                        consArgNames[i], consArgTypes[i], consArgs[i], null /*modifiers*/);
            } catch (UtilEvalError e) {
    ...
    /**
     * This method is called from the **generated class** constructor to evaluate the instance
     * initializer (instance blocks and loosely typed statements) and then the scripted constructor,
     * in the instance namespace. These activities happen in the bsh script but have side effects in
     * the generated stub class (imported instance and static variables may be initialized).
     */
    ...
    public static void initInstance(Object instance, String className, Object[] args) {
        ...
        try {
            ...
            BshMethod constructor =
                    instanceNameSpace.getMethod(constructorName, sig, true /*declaredOnly*/);
    ...
    /**
     * The class is "cold" (detached with no live interpreter static This reference) try to start a
     * new interpreter and source the script backing it.
     *
     * <p>We pass in both the fq class name and the static This ref here just to minimize the
     * generated code. All we really do here is a simple if condition for now.
     */
    ...
    /**
     * A ConstructorArgs object holds evaluated arguments for a constructor call as well as the
     * index of a possible alternate selector to invoke. This object is used by the constructor
     * switch.
     *
    ...
    /**
     * Attempt to load a script named for the class: e.g. Foo.class Foo.bsh. The script is expected
     * to (at minimum) initialize the class body. That is, it should contain the scripted class
     * definition.
     *

...
/**
 * This class is an implementation of the ClassGenerator interface which contains generally bsh
 * related code. The actual bytecode generation is done by ClassGeneratorUtil.
 *
...
public class ClassGeneratorImpl extends ClassGenerator {
    ...
    /**
     * If necessary, parse the BSHBlock for for the class definition and generate the class using
     * ClassGeneratorUtil. This method also initializes the static block namespace and sets it in
     * the class.
     */
    public static Class generateClassImpl(
        ...
        if (clas == null) {
            ...
            byte[] code = classGenerator.generateClass(Interpreter.getSaveClasses() /*init code*/);

            if (Interpreter.getSaveClasses()) saveClasses(className, code);
            else clas = bcm.defineClass(fqClassName, code);
        }
    ...
    static DelayedEvalBshMethod[] getDeclaredMethods(
        ...
        for (int child = 0; child < body.jjtGetNumChildren(); child++) {
            ...
            if (node instanceof BSHMethodDeclaration) {
                ...
                String name = md.name;
                String returnType =
                        md.getReturnTypeDescriptor(callstack, interpreter, defaultPackage);
    ...
    /**
     * A node filter that filters nodes for either a class body static initializer or instance
     * initializer. In the static case only static members are passed, etc.
     */
    static class ClassNodeFilter implements BSHBlock.NodeFilter {
        ...
        boolean isStatic(SimpleNode node) {
            ...
            if (node instanceof BSHBlock) return ((BSHBlock) node).isStatic;

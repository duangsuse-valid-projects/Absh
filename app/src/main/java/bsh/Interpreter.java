...
public class Interpreter
    ...
    public Interpreter(
        ...
        if (Interpreter.DEBUG)
            Interpreter.debug("初始化花了 " + (t2 - t1) + "ms");
    }
    ...
    public void run() {
        if (evalOnly)
            throw new RuntimeException("bsh 解释器: 没有流");
        ...
        if (interactive)
            try {
                ...
                println(
                    "BeanShell " + VERSION + " - 由 Pat Niemeyer (pat@pat.net) 开发");
            }
        ...
        while (!eof) {
            try {
                ...
                if (interactive)
                    error("模拟错误: " + e.toString());
                else
                    error("模拟: " + e.getMessage());
    ...
    throws EvalError {
        ...
        while (!eof) {
            ...
            try {
                ...
                final EvalError evalError = new EvalError("执行的文件: " + sourceFileInfo + " 内部错误: " + e.getMessage(), node, callstack);
                ...
                final EvalError evalError = new EvalError("执行的文件: " + sourceFileInfo + " 未知错误: " + e.getMessage(), node, callstack);
                evalError.initCause(e);
                throw evalError;
            } catch (TokenMgrError e) {
                final EvalError evalError = new EvalError("执行的文件: " + sourceFileInfo + " 标识符解析错误: " + e.getMessage(), node, callstack);
                evalError.initCause(e);
                throw evalError;
            } finally {
    ...
    throws EvalError {
        ...
        try {
            ...
            if (lhs.type != LHS.VARIABLE)
                throw new EvalError("不能取消, 不是变量: " + name,
